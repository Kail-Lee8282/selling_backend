...
import { CalcNowDate, nowKrDate, upDateToString, } from "../../util";
import { fullPathCategory, } from "../product.resolvers";
/**
 * 연령별 키워드 비율 반환
 * @param TrandKeyword
 * @returns
 */
function MergeTrandKeywordByGroup(TrandKeyword) {
    if (TrandKeyword.results.length > 0) {
        // 나이별로 병합
        let totalRatio = 0;
        const byAge = TrandKeyword.results[0].data.reduce((acc, curr) => {
...
function createSearchVolume(keyword, searchCnt, results) {
    const searchVolumeByMonth = [];
    // 24개월 serier 생성
    for (let i = 24; i >= 0; i--) {
        const date = new Date();
        date.setDate(1);
 ...
 * @returns
 */
export async function createSelectProductInfo(keyword, context, info) {
    try {
        const { dataSources: { naverAdAPI, naverDataLabAPI, productsDb: client }, } = context;
        if (!keyword || keyword.length <= 0)
            throw "not exist keyword";
...
const t_insertProduct = new Worker("./src/product/seeProduct/insert.product.worker.ts");
export async function InsertProduct(kwd, { dataSources: { naverAdAPI, naverDataLabAPI } }) {
    try {
        ...
        if (!product) {
            ...
            let totalPrice = 0;
            let brandCnt = 0;
            let categories = shop.items.reduce((acc, item) => {
                const curPrice = Number(item.lprice);
                // 평균가
                totalPrice += curPrice;
                //최저가
                if (lowPrice === 0 || lowPrice > curPrice)
                    lowPrice = curPrice;
                //최고가
                if (hiPrice === 0 || hiPrice < curPrice)
                    hiPrice = curPrice;
                // 브랜드 점유율
                if (item.brand) {
                    brandCnt++;
                }
                const idx = acc.findIndex((accItem) => fullPathCategory(accItem.category1, accItem.category2, accItem.category3, accItem.category4) ===
                    fullPathCategory(item.category1, item.category2, item.category3, item.category4));
                if (idx >= 0) {
                    acc[idx].count++;
                }
                else {
...
const seeProduct = async (_, { keyword: kwd }, context, info) => {
    try {
        if (kwd.length <= 0)
            throw "not exist keyword";
        const keyword = kwd.toUpperCase();
        ...
        if (findProduct) {
            if (!findProduct.searchVolumeByMonth) {
                ...
                // 2년치 일별 데이터 조회
                const { results } = await context.dataSources.naverDataLabAPI.getTotalKeywordTrandSearch(`${before2Y.getFullYear()}-${(before2Y.getMonth() + 1)
                    .toString()
                    .padStart(2, "0")}-01`, upDateToString(nowKrDate()), "date", { groupName: keyword });
                searchVolume = createSearchVolume(keyword, findProduct.totalSearch, results);
                await client.product.update({
                    ...
                    data: {
                        searchVolumeByMonth: searchVolume,
                    },
                });
            }
            return {
...
const test = async (_, { test, cid }, { dataSources: { productsDb: client, naverAdAPI } }) => {
    await getProductInfo(test);
    return {
        ok: true,
    };
};
