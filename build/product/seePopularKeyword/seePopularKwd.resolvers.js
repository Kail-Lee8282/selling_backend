import { getRankKeywords, } from "../../external/api/naver/url/naver.urlRequest";
import { dateToString, nowKrDate, upDateToString } from "../../util";
import { finalCategory } from "../product.resolvers";
import { createSelectProductInfo, InsertProduct, } from "../seeProduct/seeProduct.resolvers";
/**
 ...
 * @returns
 */
async function createPopularKwdArr(arr, cid, context, info) {
    const result = [];
    if (!arr)
        return result;
...
async function getPopularKwds(cid, context, info) {
    try {
        // 한달 기준
        ...
        let result = [];
        // DB 데이터 체크
        const findPopularKwd = await context.dataSources.productsDb.categoryPopularKwd.findMany({
            where: {
        ...
        if (findPopularKwd && findPopularKwd.length > 0) {
            ...
            // 키워드 랭킹 조회
            const getPopularKwd = await getRankKeywords(cid, dateToString(beforMonth), dateToString(beforday));
            const insertData = getPopularKwd.ranks.map((kwd) => {
                return {
                    date: dateToString(nowKrDate()),
                    keyword: kwd.keyword,
                    cid,
            ...
            await Promise.all(insertData.map(async (item) => {
                const keywordInfo = await context.dataSources.productsDb.keywords.findFirst({
                    where: {
                        keyword: item.keyword,
                    },
                });
                if (keywordInfo) {
                    ...
                    const managedKwd = await context.dataSources.naverAdAPI.getManagedKeyword(item.keyword);
            ...
            result = await createPopularKwdArr(getPopularKwd?.ranks, cid, context, info);
        }
        return result;
    }
    catch (e) {
...
const seePopularKwd = async (_, { cid, keyword }, context) => {
    try {
        // 한달 기준
        const beforMonth = nowKrDate();
        const beforday = nowKrDate();
        ...
        const { dataSources: { productsDb: client, naverAdAPI: adApi }, } = context;
        // 키워드 랭킹 조회
        let ranks = [];
        ...
        if (dbKwdRank && dbKwdRank.length > 0) {
            ...
            const getPopularKwd = await getRankKeywords(cid, dateToString(beforMonth), dateToString(beforday), page);
            ranks.push(...getPopularKwd.ranks);
            try {
                await Promise.all(ranks.map(async (item) => {
        ...
        const data = await Promise.all(ranks.map(async (kwd) => {
            ...
            if (product) {
                ...
                const categoryInfo = await getProductTopCategoryInfo(client, categories, cid);
                const item = {
                    keyword: kwd.keyword,
                    rank: kwd.rank,
                ...
                const categoryInfo = await getProductTopCategoryInfo(client, addProduct.category, cid);
                const item = {
                    keyword: kwd.keyword,
                    rank: kwd.rank,
...
async function getProductTopCategoryInfo(client, data, defaultCid) {
